...
function createOrgs() {
  CryptogenPath="./crypto/cryptogen"
  Output="crypto/crypto-config"
  if [ -d "$Output/peerOrganizations" ]; then
    rm -Rf $Output/peerOrganizations && rm -Rf $Output/ordererOrganizations
  fi
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=$CryptogenPath/crypto-config-org1.yaml --output=$Output
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=$CryptogenPath/crypto-config-orderer.yaml --output=$Output
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  echo
  echo "Generate CCP files for Orderer and Org1"

  $CryptogenPath/ccp-generate.sh
}
...
while [[ $# -ge 1 ]] ; do
  ...
  -dev )
    echo "dev mode"
    export FABRIC_CFG_PATH=${PWD}/configtx-dev
    IS_DEV_ENV="true"
    shift
    ;;
  * )
